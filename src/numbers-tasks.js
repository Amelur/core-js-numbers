...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const angle = Math.acos(scalar / (length1 * length2));
  return angle;
}
...
function getSumOfDigits(num) {
  ...
  const sum = numArr.reduce((acc, item) => +acc + +item);
  return sum;
}
...
function toExponential(number, fractionDigits) {
  const exponential = number.toExponential(fractionDigits);
  return exponential;
}
...
function toFixed(number, fractionDigits) {
  const fixedPoint = number.toFixed(fractionDigits);
  return fixedPoint;
}
